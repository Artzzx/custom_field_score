...
```
fixtures/
  GET-rest-api-3-search.json     → matches GET /rest/api/3/search
...
Call `harness.reset()` in `beforeEach` to clear API calls, fixtures, handlers, and KVS storage between tests. The framework uses singletons — your app's `import api from '@forge/api'` references the same `FakeApi` instance the harness controls.
...
harness.addFixture('GET', '/rest/api/3/search?jql=project%3DTEST', {
  status: 200,
...
| `@forge/react` | Stub UI Kit components with `data-testid="forge-{name}"`, `ForgeReconciler.render()`, `xcss()`, `useProductContext()` |
